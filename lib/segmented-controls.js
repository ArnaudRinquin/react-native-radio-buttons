...
class SegmentedControls extends React.Component {
  ...
  renderOption(config, option, selected, onSelect, index){
    ...
    const {containerBorderRadius} = config;
    let borderRadiusStyle;

    if (Platform.OS === "android" && containerBorderRadius) {
      if (this.props.options.length > 1) {
        if (index === 0) {
          borderRadiusStyle = {borderTopLeftRadius: containerBorderRadius,
                               borderBottomLeftRadius: containerBorderRadius};
        } else if (index === this.props.options.length - 1) {
          borderRadiusStyle = {borderTopRightRadius: containerBorderRadius,
                               borderBottomRightRadius: containerBorderRadius};
        }
      } else {
        borderRadiusStyle = {borderRadius: containerBorderRadius};
      }
    }
